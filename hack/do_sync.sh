...
for i in attacher,master provisioner,master resizer,master snapshotter,master; do
  ...
  if [[ ! -d pkg/${SIDECAR} ]]; then
    ...
    cat pkg/${SIDECAR}/go.mod | { grep "replace " || [[ $? == 1 ]]; } | sed 's/v0.35.0/v0.35.2/g' | sed 's#external-snapshotter/client/v8 => ./client#csi-sidecars/pkg/snapshotter/client/v8 => ./pkg/snapshotter/client#' >>tmp/gomod-replace.txt
